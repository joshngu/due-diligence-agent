...
# 2. Get API Keys (from environment variables first, then UI fallback)
openai_api_key = os.getenv("OPENAI_API_KEY")
tavily_api_key = os.getenv("TAVILY_API_KEY")

# Sidebar for API Keys (only show if not in environment)
with st.sidebar:
...
if not openai_api_key or not tavily_api_key:
    st.warning("Please enter your API keys in the sidebar or set them in .envdev file to continue.")
    st.stop()
